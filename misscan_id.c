...
struct Node * createNode(struct Node * parentNode,struct state * thisState,int arcCost){
    ...
    newNode->next = NULL;
    if(parentNode == NULL) {
        newNode->costFromSourceToNode = 0;
        newNode->depthFromRoot = 0;
    }
    else {
        newNode->costFromSourceToNode = parentNode->costFromSourceToNode + arcCost;
        newNode->depthFromRoot = parentNode->depthFromRoot+1;
    }
}
