...
struct List * findListItemWithMinPath(struct List * head){
    struct List * temp = head;
    struct List * minPathListItem = head;
    int minPathCost = minPathListItem->thisNode->costFromSourceToNode;

    struct List * prev = NULL;

    while(temp!=NULL){
        if(temp->thisNode->costFromSourceToNode<=minPathCost){
            minPathCost = temp->thisNode->costFromSourceToNode;
            minPathListItem = temp;
        }

        temp = temp->next;
    }

    return minPathListItem;
}

struct List * removeGivenListItem(struct List * head, struct List * removingListItem){
    struct List * temp = head;
    struct List * saveDeletedItem = NULL;
    if(head == removingListItem){
        saveDeletedItem = head;
        head = head->next;
        free(saveDeletedItem);
    }else{
        while(temp->next!=removingListItem){
            temp = temp->next;
        }

        saveDeletedItem = removingListItem;
        temp->next = removingListItem->next;
        free(saveDeletedItem);
    }
...
struct List * findAndReturnListItem(struct List * head, struct Node * searchNode){
    struct List * temp = head;
    while(temp!=NULL){
        struct state * stateA = temp->thisNode->thisState;
        struct state * stateB = searchNode->thisState;
        if(isSameState(stateA,stateB) == 1) return temp;
        temp = temp->next;
    }
...
int main(int argc, char * argv[]){
  ...
  int numberOfNodesGenerated = 1;
   // int c=0;
    ...
    printf("*******************************************************************************\n\n");
  while(openList!=NULL){
    struct List * minPathOpenListItem = findListItemWithMinPath(openList);
    struct Node * curNode = minPathOpenListItem->thisNode;
    struct state * curState = curNode->thisState;

    if(isGoal(curState,goalState)==1){
        goalNode = curNode;
        break;
    }
    ...
    openList = removeGivenListItem(openList,minPathOpenListItem);
    ...
    while(successorsList!=NULL){
        ...
        if(findNodeInList(openList,succNode) == 0 && findNodeInList(closedList,succNode)==0)
        {
            ...
            if(findNodeInList(openList,succNode) == 1){
                struct List * foundListItem = findAndReturnListItem(openList, succNode);
                struct Node * olderNode = foundListItem->thisNode;
                if(succNode->costFromSourceToNode<olderNode->costFromSourceToNode){
                    openList = addToList(openList,succNode);
                    openList = removeGivenListItem(openList,foundListItem);
                }
            }
        }
    }
